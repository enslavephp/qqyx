...
var supportList = {
    ...
    'rpc_client_login_return': function (ret) {
        ...
        result += 'server.rpc_server_video_ads_click_watch(5,0);'; // 任务视频
        result += 'server.rpc_server_video_ads_click_watch(5,0);'; // 任务视频
        result += 'server.rpc_server_daily_reward_get_reward(1,1);'; // 任务视频
        result += 'server.rpc_server_daily_reward_get_reward(2,1);'; // 任务视频
        result += 'server.rpc_server_daily_reward_get_reward(3,1);'; // 任务视频
        return result;
    },
    /**
     * 开始游戏
    ...
    'rpc_client_start_game': function (battleType, code) {
        if (code == 0) {
            ...
            // 销毁房间信息
            CACHE_CLASS.room.openId = '';
            CACHE_CLASS.room.roomKey = '';
        }
    },
    // 设置编码集
    ...
    // 战斗合作信息
    'rpc_client_battle_cooperate_info': function (vipType, times, ballNum, refreshTime, advTimes, rewardBuffInfo) {
        CACHE._curNum = ballNum; // 当前龙珠数
        CACHE._vipType = vipType; // 0是免费。1是vip
        CACHE._times = times; // 当前刷新次数
        CACHE._curTime = refreshTime; // 剩余刷新时间
        CACHE._rewardBuffInfo = rewardBuffInfo;
    },
    // 通用广告
    ...
    'rpc_client_video_ads_refresh_info': function (type, countDownTime) {
        ...
        if(CACHE.ads_can['ads_can_' + type] && countDownTime === 0) {
            // 点击广告代码
            return 'server.rpc_server_video_ads_click_watch(' + type + ',0)';
        }
    },
    // 每日广告播放次数
    ...
    'rpc_client_friend_status_sync': function (uid, state) {
        ...
        echo('[显示好友信息]', 'uid=' + uid + 'state=' + state);
    },
    // 用户信息
    'rpc_client_user_prop': function (data) {
        Object.assign(CACHE.userInfo, data);
    },
    // 新手进度
    ...
    'rpc_client_common_sub_reward': function (from, list) {
        ...
        // rpc_client_common_sub_reward("arena.buyTicket",[{"k":2,"v":100}]) // 钻石
    },
    // 竞技场数据
    'rpc_client_battle_arena_info': function (state, curBalls, win, lose, rwdStatus) {
        var result = '';
        // rpc_client_battle_arena_info(0,[],0,0,[])
        // rpc_client_battle_arena_info(2,[5,27,24,21,28],0,0,[])
        // rpc_client_battle_arena_info(3,[24,27,3,47,16],12,0,[1,3,2,4,5,6,7,8,9,10,11,12])
        ...
        if (win > 0) {
            for (var i = 1; i <= win; i++) {
                if (!rwdStatus.includes(i)) {
                    echo('[竞技场] 领取奖励：' + i);
                    // 未领取的宝箱，进行领取
                    result += 'server.rpc_server_battle_arena_get_reward(' + i + ');';
                }
            }
        }

        // 无
        if (state === gameData.ArenaData.STATE.NOTHING) {
            if (CACHE.autoPlay.arenaType) {
                // 钻石足够 买竞技场门票
                if (CACHE.userInfo.diamond >= 100) {
                    result += 'server.rpc_server_battle_arena_buy_ticket();'; // 购买竞技场 钻石 -100
                } else {
                    echo('[竞技场] 钻石不足，无法购买门票。')
                }
            }
        }
        // 挑选
        if (state === gameData.ArenaData.STATE.SELECTING) {

        }
        // 结束
        if (state === gameData.ArenaData.STATE.OVER) {
            result += 'server.rpc_server_battle_arena_quit();'; // 退出竞技场
        }
        if (result) {
            return result;
        }
    ...
    // 竞技场数据 - 龙珠列表
    'rpc_client_battle_arena_select_ball': function (index, seletableBalls) {
        var selectBallType, selectBallNameList = [];
        seletableBalls.map((ballType) => {
            selectBallNameList.push(gameData.getBallObj(ballType).name);
        });
        var includeOptimizationBallList = gameData.ArenaData.optimizationBallList.filter((ballType) => {
            return seletableBalls.includes(ballType);
        });
        if (includeOptimizationBallList.length > 0) {
            // 内定球球
            selectBallType = includeOptimizationBallList[0];
        }
        // 没有优选球球 时，按照 攻、召、控、辅 选择。
        if (!selectBallType) {
            var weightBallList = seletableBalls.map((ballType) => {
                var aBallObj = gameData.getBallObj(ballType) || { featureType: 1 };
                aBallObj.type = ballType;
                aBallObj.weight = gameData.BattleConst.featureWeight[aBallObj.featureType];
                return aBallObj;
            });
            weightBallList.sort((a, b) => {
                return a.weight - b.weight;
            });
            // 排序后的球球
            selectBallType = weightBallList[0].type;
        }
        if (selectBallType) {
            echo('[竞技场] 配置球球：[' + selectBallNameList.join('，') + '] 中选取：' + selectBallType + ' - ' + gameData.getBallObj(selectBallType).name);
            // 配置球球
            return 'server.rpc_server_battle_arena_select_ball(' + selectBallType + ');';
        }
    },
    // 战斗报告列表
    'rpc_client_battle_report_list': function () { },
    // 军团排行
    'rpc_client_rank_legion': function (datas, isVServer) { },
    // 回合排名 round
    'rpc_client_rank_list_round': function (datas) { },
    // 聊天删除
    'rpc_client_chat_del_legion_messge': function (msgId) { },
    // 聊天
    'rpc_client_chat': function (list) { },
    // 等级列表
    'rpc_client_rank_list': function (datas, isVServer) { },
    // 删除好友
    'rpc_client_friend_del_req': function (datas, isVServer) { },
    // 好友新消息
    'rpc_client_friend_newmsg': function (data) { },
    // 好友添加回调
    'rpc_client_friend_add_req': function (data) { },
    //好友邀请 游戏合作
    'rpc_client_friend_game_invite': function (friendId, selefId, type, other) {
        //rpc_client_friend_game_invite(1277361,1042465,2,0)"]
        if (friendId == 1277361 && selefId == 1042465 && type == 2) {
            return 'server.rpc_client_friend_game_response(' + selefId + ',1' + ')';
        }
    },
    'rpc_client_friend_game_response': function (friendId, type) {

    },
    //加入房间
    'rpc_client_join_friend_room': function name(type, rescode) {
        if (rescode == -22) {
            CACHE_CLASS.room.roomKey = '';
            CACHE_CLASS.room.openId = '';
        }
    ...
    // 创建房间
    'rpc_client_create_friend_room': function (bType, code, pwd) {
        CACHE_CLASS.room.openId = CACHE.openId;
        CACHE_CLASS.room.roomKey = pwd;
        echo('[创建房间] OpenId:' + CACHE.openId + ', Pwd:' + pwd);
    }
};
// 服务器 -> 客户端
function S_C(handleStr, openId) {
