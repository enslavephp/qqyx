...
var supportList = {
    ...
    'rpc_client_brpc_proto': function (content) {
        // rpc_client_brpc_proto("{\"param\":{\"statistics\":{1635225:{},2626156:{}},\"seed\":0,\"concede\":1,\"round\":1,\"win\":2,\"record\":[]},\"action\":\"battleOver\"}")
        // var json = JSON.parse(content); // 解析会报错
        /*CTRL_ACTION = {
        ...
        var execResult = /\"action\":\"(\S+)\"/.exec(content);
    ...
    'rpc_client_fight_frame_end': function () {
        var result = "", i, key, keyList, valueList, _minGrade, allPowenfulBallList, temp1,
        ...
        if (playInfo.cfg) {
            ...
            var getGradeSp = function (minGrade) {
                if (minGrade) {
                    return gameData.BattleConst.DragonBallUpgradeCost[minGrade.grade]; // 根据等级取升级所需 SP
                }
            };
            ...
            var getMinGrade = function () {
                ...
                valueList = Object.values(playInfo.ballsGrade).filter((item) => {
                    // 在不升级球球列表中，不处理升级
                    if (gameData.BattleConst.notUpgrade.includes(item.dbType)) {
                        return false;
                    }
                    // 非满级 且 不是辅助球球
                    return item.grade <= 4 && gameData.getBallObj(item.dbType).featureType !== 3;
                });
                // 有需要升级 Lv 的球球
                if (valueList.length > 0) {
                    ...
                    valueList = Object.values(playInfo.ballsGrade).filter((item) => {
                        ...
                        // 辅助球球 且 不在可以升级的辅助球球白名单中 不升级
                        if (gameData.getBallObj(item.dbType).featureType === 3
                            && !gameData.BattleConst.assistUpgrade.includes(item.dbType)) {
                            return false;
                        }
                        // 取球球 升级所需 sp
            ...
            var mergeBall = function (_keyList) {
                ...
                for (i = 0; i < _keyList.length; i++) { // 循环所有球球，寻找可合并的
                    ...
                    if (ballItem) {
                        ...
                        var aBallObj = CACHE.getBallById(key);
                        var bBallObj = CACHE.getBallById(ballItem.ballId);
                        echo('[球球合并] [' + aBallObj.star + '星] ' + aBallObj.ballName + '-' + key + ' -> ' + bBallObj.ballName + '-' + ballItem.ballId);
                        evalStr += 'battle_server.rpc_server_fight_ball_create();'; // 创建球球
                        return evalStr;
                    }
                }
            };
            // 取棋盘是否存在万能球球 或 优先处理球球
            var getAllPowenfulBallList = function () {
                return Object.values(ballList).filter((ballItem) => {
                    // 祭品球球，繁衍球球、暗杀球球,召唤球球sjc
                    if ([16, 33, 35, 40].includes(ballItem.ballType)) {
                        return true;
                    }
                    // 棋盘包含万能球球
            ...
            if (_minGrade) { // 有等级最低球球
                ...
                if (CACHE.battle.battleType === 1) {//竞技模式中 如果飓风大于10个  可以考虑优先升级等级
                    var jufNum = 0;
                    Object.values(CACHE.battle.self.ballList).filter((ballItem) => {
                        if (ballItem.ballType == 38) {
                            jufNum += ballItem.star
                        }
                    });
                ...
                if (keyList.length >= playInfo.ballMaxNum) {
                    // 200+ SP 升级 Lv.3
                    if (playInfo.cfg.cost >= 100 && _minGrade.grade < 2) {
                        // 升级球球
                        result += upGrade();
                        return result;
                    } else if (playInfo.cfg.cost >= 150 && _minGrade.grade < 3) {
                        // 升级球球
                        result += upGrade();
                        return result;
                    } else if (playInfo.cfg.cost >= 200 && _minGrade.grade < 4) {
                        // 升级球球
                        result += upGrade();
                        return result;
                    } else if (playInfo.cfg.cost >= 250 && _minGrade.grade < 5) {
                        // 升级球球
                        result += upGrade();
                        return result;
                    }
            ...
            if (playInfo.cfg.sp >= playInfo.cfg.cost) {
                ...
                // 总球数少于 15 个可以创建球球
                if (keyList.length < playInfo.ballMaxNum) {
                    result += 'battle_server.rpc_server_fight_ball_create();'; // 创建球球
                    return result;
                } else {
                    // 球球满了
                    temp1 = mergeBall(keyList);
                ...
                if (keyList.length === 4) { // 开局四抽
                    ...
                    if (keyList.length === 1) {
                        ...
                        if (gameData.BattleConst.startToMerge.includes(keyList[0])) {
                            result += mergeBall([ballList[0].ballId]); // 合并球球
                            return result;
                        }
                    }
                }
    ...
    'rpc_client_fight_boss_trailer': function (bossType) {
        ...
        echo("BOSS预告提示===" + bossType);
    },
    // 轮次开始
    'rpc_client_fight_round_begin': function (time, round) {
        // rpc_client_fight_round_begin(120,1)
        // time = BOSS 来临倒计时？
        //提前判断暗杀boss 在来临前一回合合并1星球球
        var isKillBossComing = false;
        if (round <= 50) {
            if (round == 49) {//49回合 71 之后每3回合 出一次BOSS
                isKillBossComing = true;
                bossTypeWithRound = 105;
            }
        } else if (round <= 71) {
            if (round == 70) {//49回合 71 之后每3回合 出一次BOSS
                isKillBossComing = true;
                bossTypeWithRound = 105;
            }
        } else {
        ...
        var currentGetKeys = 0;
        if (round > 50) {
            currentGetKeys = (round - 50) * 3 + 50;
        }else{
            currentGetKeys = round;
        }
        
        echo('开始第' + round + '回合  ' + CACHE.userInfo.name + '当前钥匙:' + (CACHE._curNum + currentGetKeys));
    },
    // 轮次结束
    ...
    'rpc_client_fight_monster_create': function (monsterId, monsterType, monsterBaseInfo) {
        ...
        if (CACHE.battle.selfIndex === monsterBaseInfo.side) {
            ...
            if (monsterType >= 101 && monsterType <= 105) {
                ...
                CACHE.battle.bossTrailer = monsterType;
            }
        }
    },
    // 销毁怪物
...
// 战斗：服务器 -> 客户端
function BS_C(handleStr, openId) {
