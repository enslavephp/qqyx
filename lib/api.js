...
function Realize(request, response) {
    ...
    getPost(request, response).then((post) => {
        ...
        handleMsg(json.msg, json.openId);
...
function beforeHandleMsg(str) {
    ...
    var shieldStrList = [
        ...
        'disconnect battle server socket.',
        '该商品列表在表中是属于id',
    ];
...
function handleMsg(msg, openId) {
    ...
    if(msgList.length > 0) {
        if(/^\[\w+\]:$/.test(msgList[0])) {
            ...
            if(msgList.length > 0) {
                ...
                eval = acquireEval(msgList, openId);
    ...
    // 处理过的消息不提示，由处理方法自行提示
    if(!eval) {
        echo('[HandleMsg]', openId?'['+openId+']':'', logStr);
    }
}

// 处理消息并返回 eval
function acquireEval(msgList, openId) {
    var evalStr, msgItem;
    var handleFunc = {
        // 客户端 -> 服务器
        'C_S': C_S,
    ...
    if(msgList.length > 0) {
        msgItem = msgList[0];
        if(/^\[C->S\]/.test(msgItem)) {
            evalStr = handleFunc.C_S(msgItem.substr(7), openId);
        } else if(/^\[S->C\]/.test(msgItem)) {
            evalStr = handleFunc.S_C(msgItem.substr(7), openId);
        } else if(/^\[BS->C\]/.test(msgItem)) {
            evalStr = handleFunc.BS_C(msgItem.substr(8), openId);
        } else if(/^\[C->BS\]/.test(msgItem)) {
            evalStr = handleFunc.C_BS(msgItem.substr(8), openId);
        }
    }
    return evalStr;
}
